...
static uint8_t channel_curr;
static uint8_t power_curr;
...
static void saProcessResponse(uint8_t *buf, int len)
{
    ...
    switch(resp) {
        ...
        case SA_CMD_GET_SETTINGS: // Version 1 Get Settings
            ...
            channel_curr = sa_chan;
            if (first_response) {
                masterConfig.vtx_power = power_curr;
                masterConfig.vtx_channel = channel_curr;
                masterConfig.vtx_valid = 1;
            }
...
void smartAudioSetBandChan(int channel)
{
...
void smartAudioProcess(uint32_t now)
{
    ...
    if (sa_vers == 0) {
        ...
        if (masterConfig.vtx_power != power_curr) { 
            smartAudioSetPowerByIndex(masterConfig.vtx_power);

            sa_vers = 0;
            masterConfig.vtx_valid = 0;
        }

        if (masterConfig.vtx_channel != channel_curr) {
            smartAudioSetBandChan(masterConfig.vtx_channel);
            sa_vers = 0;
            masterConfig.vtx_valid = 0;
        }
